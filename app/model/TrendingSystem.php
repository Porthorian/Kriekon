...
class TrendingSystem extends Model
{
	...
	public function fetchConfidentThreads()
    {
        ...
        $sql = 'SELECT thread_id, ((thread_upvotes + 1.920 / (thread_upvotes + thread_downvotes) - 
                   1.96 * SQRT((thread_upvotes * thread_downvotes) / (thread_upvotes + thread_downvotes) + 0.9604) / 
                          (thread_upvotes + thread_downvotes)) / (1 + 3.8416 / (thread_upvotes + thread_downvotes))) 
            AS confidence 
            FROM forum_thread 
            WHERE thread_upvotes + thread_downvotes > 0 
            ORDER BY confidence DESC';
    ...
    public function fetchHotThreads($limit = null, $limit_offset = null)
    {
        ...
        if($limit == null && $limit_offset == null)
		{
            ...
            FROM forum_thread
            ORDER BY
                LOG10( ABS( thread_upvotes - thread_downvotes) + 1 ) *
                SIGN( thread_upvotes - thread_downvotes ) + 
	...
	/*public function increaseUpvotes($user_id, $thread_id = null, $article_id = null) 
	{
		...
		if($result == false){
			if($thread_id != null)
				$db->insert('forum_upvote_downvote', ['upvoted'=>1,'user_id'=>$user_id, 'thread_id'=>$thread_id]);
			else
				$db->insert('article_upvote_downvote', ['upvoted'=>1, 'user_id'=>$user_id, 'article_id'=>$article_id]);
		} else {
			if($thread_id != null)
			{
				if($result['upvoted'] == 0)
				{
					$db->update('forum_upvote_downvote', ['upvote_downvote_id'=>$result['upvote_downvote_id']], ['upvoted'=>1]);
					return 0;
					...
					$db->update('forum_upvote_downvote', ['upvote_downvote_id'=>$result['upvote_downvote_id']], ['upvoted'=>0]);
					return 2;
				}
			}
			else
	...
	public function increaseUpvotes($user_id, $thread_id = null, $article_id = null)
	{
		...
		$upvote_downvote_time = (date('Y-m-d H:i:s', time()));
		if($thread_id != null)
			$db->insert('forum_upvote_downvote', ['upvoted'=>$upvoted_on, 'user_id'=>$user_id, 'thread_id'=>$thread_id, 'upvote_downvote_time'=>$upvote_downvote_time]);
		elseif($article_id != null)
			$db->insert('article_upvote_downvote', ['upvoted'=>$upvoted_on, 'user_id'=>$user_id, 'article_id'=>$article_id, 'upvote_downvote_time'=>$upvote_downvote_time]);
	}
	...
	{ 
		...
		if($result == false){
			if($thread_id != null)
				$db->insert('forum_upvote_downvote', ['downvoted'=>1,'user_id'=>$user_id, 'thread_id'=>$thread_id]);
			else
		...
		} else 
		{
			if($thread_id != null)
			{
				...
				{
					$db->update('forum_upvote_downvote', ['upvote_downvote_id'=>$result['upvote_downvote_id']], ['downvoted'=>1]);
				} elseif ($result['downvoted'] == 1 && $result['upvoted'] == 0){
					$db->delete('forum_upvote_downvote', ['upvote_downvote_id'=>$result['upvote_downvote_id']]);
				} elseif ($result['downvoted'] == 1 && $result['upvoted'] == 1){
					$db->update('forum_upvote_downvote', ['upvote_downvote_id'=>$result['upvote_downvote_id']], ['downvoted'=>0]);
				}
			}
			else
			{
	...
	public function increaseDownvotes($user_id, $thread_id = null, $article_id = null)
	{
		...
		$upvote_downvote_time = (date('Y-m-d H:i:s', time()));
		if($thread_id != null)
			$db->insert('forum_upvote_downvote', ['downvoted'=>$downvoted_on, 'user_id'=>$user_id, 'thread_id'=>$thread_id, 'upvote_downvote_time'=>$upvote_downvote_time]);
		elseif($article_id != null)
			$db->insert('article_upvote_downvote', ['downvoted'=>$downvoted_on, 'user_id'=>$user_id, 'article_id'=>$article_id, 'upvote_downvote_time'=>$upvote_downvote_time]);
	}
